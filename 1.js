...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .terminal-prompt {
            color: #007acc; margin-right: 10px;
            font-weight: bold;
        }
        .terminal-input {
        ...
        .terminal-cursor {
            ...
            background: #007acc; animation: blink 1s infinite;
            margin-left: 2px;
        }
        @keyframes blink {
            0%, 50% { opacity: 1; }
        ...
        .terminal-success {
            color: #388e3c; margin: 5px 0;
        }
    </style>
</head>
<body>
    <div style="position: fixed; top: 20px; left: 20px; z-index: 1000;">
        <select id="languageSelector" style="background: #fff; border: 1px solid #ddd; color: #333; padding: 8px 12px; font-family: 'Courier New', monospace; font-size: 14px; cursor: pointer;" onchange="changeLanguage(this.value)">
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .client-btn {
            ...
            padding: 12px 16px; color: #3498db;
        ...
        .generate-btn {
            background: #3498db;
            border: 1px solid #3498db; padding: 12px 24px;
        ...
        .subscription-url {
            background: #f8f9fa;
            border: 1px solid #e1e8ed; padding: 12px;
            word-break: break-all; font-family: "Courier New", monospace;
            color: #2c3e50; margin-top: 20px; display: none;
            border-radius: 6px;
...
<body>
    <div style="position: fixed; top: 20px; left: 20px; z-index: 1000;">
        <select id="languageSelector" style="background: #fff; border: 1px solid #ddd; color: #333; padding: 8px 12px; font-family: inherit; font-size: 14px; cursor: pointer; border-radius: 4px;" onchange="changeLanguage(this.value)">
            <option value="zh" ${!isFarsi ? 'selected' : ''}>🇨🇳 中文</option>
            <option value="fa" ${isFarsi ? 'selected' : ''}>🇮🇷 فارسی</option>
        </select>
    </div>
    <div class="container">
        ...
        <div class="card">
            ...
            <div id="systemStatus" style="margin: 20px 0; padding: 15px; background: #f8f9fa; border: 1px solid #e1e8ed; position: relative; overflow: hidden;">
                    <div style="color: #2c3e50; margin-bottom: 15px; font-weight: bold;">[ ${t.checking} ]</div>
                    <div id="regionStatus" style="margin: 8px 0; color: #2c3e50; font-family: inherit;">${t.workerRegion}${t.checking}</div>
                    <div id="geoInfo" style="margin: 8px 0; color: #7f8c8d; font-family: inherit; font-size: 0.9rem;">${t.detectionMethod}${t.checking}</div>
                    <div id="backupStatus" style="margin: 8px 0; color: #2c3e50; font-family: inherit;">${t.proxyIPStatus}${t.checking}</div>
                    <div id="currentIP" style="margin: 8px 0; color: #2c3e50; font-family: inherit;">${t.currentIP}${t.checking}</div>
                    <div id="regionMatch" style="margin: 8px 0; color: #2c3e50; font-family: inherit;">${t.regionMatch}${t.checking}</div>
                    <div id="selectionLogic" style="margin: 8px 0; color: #7f8c8d; font-family: inherit; font-size: 0.9rem;">${t.selectionLogic}${t.selectionLogicText}</div>
            </div>
        </div>
        <div class="card" id="configCard" style="display: none;">
                <h2 class="card-title">${t.configManagement}</h2>
            <div id="kvStatus" style="margin-bottom: 20px; padding: 10px; background: #f8f9fa; border: 1px solid #e1e8ed; color: #2c3e50;">
                    ${t.kvStatusChecking}
            </div>
            <div id="configContent" style="display: none;">
                <form id="regionForm" style="margin-bottom: 20px;">
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.specifyRegion}</label>
                        <select id="wkRegion" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            ...
                            <small id="wkRegionHint" style="color: #e74c3c; font-size: 0.85rem; display: none;">⚠️ ${t.customIPDisabledHint}</small>
                    </div>
                        <button type="submit" style="background: #3498db; border: 1px solid #3498db; padding: 10px 20px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; margin-right: 10px; border-radius: 4px;">${t.saveRegion}</button>
                </form>
                <form id="otherConfigForm" style="margin-bottom: 20px;">
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.protocolSelection}</label>
                        <div style="padding: 15px; background: #f8f9fa; border: 1px solid #e1e8ed; border-radius: 6px;">
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="ev" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enableVLESS}</span>
                                </label>
                            </div>
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="et" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enableTrojan}</span>
                                </label>
                            </div>
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="ex" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enableXhttp}</span>
                                </label>
                            </div>
                            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e1e8ed;">
                                    <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-size: 0.95rem;">${t.trojanPassword}</label>
                                    <input type="text" id="tp" placeholder="${t.trojanPasswordPlaceholder}" style="width: 100%; padding: 8px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 13px; border-radius: 4px;">
                                    <small style="color: #7f8c8d; font-size: 0.8rem; display: block; margin-top: 5px;">${t.trojanPasswordHint}</small>
                            </div>
                                <small style="color: #7f8c8d; font-size: 0.85rem; display: block; margin-top: 10px;">${t.protocolHint}</small>
                                <button type="button" id="saveProtocolBtn" style="margin-top: 15px; background: #3498db; border: 1px solid #3498db; padding: 8px 16px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; border-radius: 4px; width: 100%;">${t.saveProtocol}</button>
                        </div>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.customHomepage}</label>
                            <input type="text" id="customHomepage" placeholder="${t.customHomepagePlaceholder}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.customHomepageHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.customPath}</label>
                            <input type="text" id="customPath" placeholder="${isFarsi ? 'مثال: /mypath یا خالی بگذارید تا از UUID استفاده شود' : '例如: /mypath 或留空使用 UUID'}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${isFarsi ? 'مسیر اشتراک سفارشی. اگر خالی بگذارید از UUID به عنوان مسیر استفاده می‌شود.' : '自定义订阅路径。留空则使用 UUID 作为路径。'}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.customIP}</label>
                            <input type="text" id="customIP" placeholder="${isFarsi ? 'مثال: 1.2.3.4:443' : '例如: 1.2.3.4:443'}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${isFarsi ? 'آدرس و پورت ProxyIP سفارشی' : '自定义ProxyIP地址和端口'}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.preferredIPs}</label>
                            <input type="text" id="preferredIPs" placeholder="${isFarsi ? 'مثال: 1.2.3.4:443#گره هنگ‌کنگ,5.6.7.8:80#گره آمریکا,example.com:8443#گره سنگاپور' : '例如: 1.2.3.4:443#香港节点,5.6.7.8:80#美国节点,example.com:8443#新加坡节点'}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${isFarsi ? 'فرمت: IP:پورت#نام گره یا IP:پورت (بدون # از نام پیش‌فرض استفاده می‌شود). پشتیبانی از چندین مورد، با کاما جدا می‌شوند. <span style="color: #e67e22;">IP های اضافه شده از طریق API به طور خودکار در اینجا نمایش داده می‌شوند.</span>' : '格式: IP:端口#节点名称 或 IP:端口 (无#则使用默认名称)。支持多个，用逗号分隔。<span style="color: #e67e22;">API添加的IP会自动显示在这里。</span>'}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.preferredIPsURL}</label>
                            <input type="text" id="preferredIPsURL" placeholder="${isFarsi ? 'پیش‌فرض: https://raw.githubusercontent.com/qwer-search/bestip/refs/heads/main/kejilandbestip.txt' : '默认: https://raw.githubusercontent.com/qwer-search/bestip/refs/heads/main/kejilandbestip.txt'}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${isFarsi ? 'URL منبع لیست IP ترجیحی سفارشی، اگر خالی بگذارید از آدرس پیش‌فرض استفاده می‌شود' : '自定义优选IP列表来源URL，留空则使用默认地址'}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.socks5Config}</label>
                            <input type="text" id="socksConfig" placeholder="${isFarsi ? 'مثال: user:pass@host:port یا host:port' : '例如: user:pass@host:port 或 host:port'}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${isFarsi ? 'آدرس پروکسی SOCKS5، برای انتقال تمام ترافیک خروجی استفاده می‌شود' : 'SOCKS5代理地址，用于转发所有出站流量'}</small>
                    </div>
                        <button type="submit" style="background: #3498db; border: 1px solid #3498db; padding: 10px 20px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; margin-right: 10px; border-radius: 4px;">${t.saveConfig}</button>
                </form>
                
                    <h3 style="color: #2c3e50; margin: 20px 0 15px 0; font-size: 1.2rem;">${t.advancedControl}</h3>
                <form id="advancedConfigForm" style="margin-bottom: 20px;">
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.subscriptionConverter}</label>
                            <input type="text" id="scu" placeholder="${t.subscriptionConverterPlaceholder}" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.subscriptionConverterHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.builtinPreferred}</label>
                        <div style="padding: 15px; background: #f8f9fa; border: 1px solid #e1e8ed; border-radius: 6px;">
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="epd" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enablePreferredDomain}</span>
                                </label>
                            </div>
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="epi" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enablePreferredIP}</span>
                                </label>
                            </div>
                            <div style="margin-bottom: 10px;">
                                <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                    <input type="checkbox" id="egi" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 1rem;">${t.enableGitHubPreferred}</span>
                                </label>
                            </div>
                                <small style="color: #7f8c8d; font-size: 0.85rem; display: block; margin-top: 10px;">${t.builtinPreferredHint}</small>
                        </div>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">优选IP筛选设置</label>
                        <div style="padding: 15px; background: #f8f9fa; border: 1px solid #e1e8ed; border-radius: 6px;">
                            <div style="margin-bottom: 15px;">
                                <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">IP版本选择</label>
                                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                        <input type="checkbox" id="ipv4Enabled" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 0.95rem;">IPv4</span>
                                    </label>
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                        <input type="checkbox" id="ipv6Enabled" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 0.95rem;">IPv6</span>
                                    </label>
                                </div>
                            </div>
                            <div style="margin-bottom: 10px;">
                                <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">运营商选择</label>
                                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                        <input type="checkbox" id="ispMobile" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 0.95rem;">移动</span>
                                    </label>
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                        <input type="checkbox" id="ispUnicom" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 0.95rem;">联通</span>
                                    </label>
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #2c3e50;">
                                        <input type="checkbox" id="ispTelecom" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                        <span style="font-size: 0.95rem;">电信</span>
                                    </label>
                                </div>
                            </div>
                                <small style="color: #7f8c8d; font-size: 0.85rem; display: block; margin-top: 10px;">选择要使用的IP版本和运营商，未选中的将被过滤</small>
                        </div>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.allowAPIManagement}</label>
                        <select id="apiEnabled" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                                <option value="">${t.apiEnabledDefault}</option>
                                <option value="yes">${t.apiEnabledYes}</option>
                        </select>
                            <small style="color: #e67e22; font-size: 0.85rem;">${t.apiEnabledHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.regionMatching}</label>
                        <select id="regionMatching" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                                <option value="">${t.regionMatchingDefault}</option>
                                <option value="no">${t.regionMatchingNo}</option>
                        </select>
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.regionMatchingHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.downgradeControl}</label>
                        <select id="downgradeControl" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                                <option value="">${t.downgradeControlDefault}</option>
                                <option value="no">${t.downgradeControlNo}</option>
                        </select>
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.downgradeControlHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.tlsControl}</label>
                        <select id="portControl" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                                <option value="">${t.tlsControlDefault}</option>
                                <option value="yes">${t.tlsControlYes}</option>
                        </select>
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.tlsControlHint}</small>
                    </div>
                    <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #2c3e50; font-weight: 500;">${t.preferredControl}</label>
                        <select id="preferredControl" style="width: 100%; padding: 10px; background: #fff; border: 1px solid #ddd; color: #333; font-family: inherit; font-size: 14px; border-radius: 4px;">
                                <option value="">${t.preferredControlDefault}</option>
                                <option value="yes">${t.preferredControlYes}</option>
                        </select>
                            <small style="color: #7f8c8d; font-size: 0.85rem;">${t.preferredControlHint}</small>
                    </div>
                        <button type="submit" style="background: #3498db; border: 1px solid #3498db; padding: 10px 20px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; margin-right: 10px; border-radius: 4px;">${t.saveAdvanced}</button>
                </form>
                <div id="currentConfig" style="background: #f8f9fa; border: 1px solid #e1e8ed; padding: 15px; margin: 10px 0; font-family: 'Courier New', monospace; color: #2c3e50; border-radius: 4px;">
                        ${t.loading}
                </div>
                <div id="pathTypeInfo" style="background: #f8f9fa; border: 1px solid #e1e8ed; padding: 15px; margin: 10px 0; font-family: 'Courier New', monospace; color: #2c3e50; border-radius: 4px;">
                        <div style="font-weight: bold; margin-bottom: 8px; color: #27ae60;">${t.currentConfig}</div>
                        <div id="pathTypeStatus">${t.checking}</div>
                </div>
                    <button onclick="loadCurrentConfig()" style="background: #3498db; border: 1px solid #3498db; padding: 10px 20px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; margin-right: 10px; border-radius: 4px;">${t.refreshConfig}</button>
                    <button onclick="resetAllConfig()" style="background: #e74c3c; border: 1px solid #e74c3c; padding: 10px 20px; color: white; font-family: inherit; font-weight: 500; cursor: pointer; border-radius: 4px;">${t.resetConfig}</button>
            </div>
            <div id="statusMessage" style="display: none; padding: 10px; margin: 10px 0; border: 1px solid #3498db; background: #e8f4fd; color: #2c3e50; border-radius: 4px;"></div>
        </div>
        
        <div class="card">
                <h2 class="card-title">${t.relatedLinks}</h2>
            <div style="text-align: center; margin: 20px 0;">
                    <a href="https://github.com/byJoey/cfnew" target="_blank" style="color: #3498db; text-decoration: none; margin: 0 20px; font-size: 1rem;">${t.githubProject}</a>
                <a href="https://www.youtube.com/@joeyblog" target="_blank" style="color: #3498db; text-decoration: none; margin: 0 20px; font-size: 1rem;">YouTube @joeyblog</a>
            </div>
        </div>
    </div>
    <script>
                ...
                try {
                    ...
                    if (data.region === 'CUSTOM') {
                        ...
                        // 获取自定义IP的详细信息
                            ...
                            geoInfo.innerHTML = t.detectionMethod + '<span style="color: #e67e22;">⚙️ ' + t.customIPMode + '</span>';
                            regionStatus.innerHTML = t.workerRegion + '<span style="color: #e67e22;">🔧 ' + t.customIPModeDesc + '</span>';
                        
                        // 显示自定义IP配置状态，包含具体IP
                            if (backupStatus) backupStatus.innerHTML = t.proxyIPStatus + '<span style="color: #e67e22;">🔧 ' + t.usingCustomProxyIP + customIPInfo + '</span>';
                            if (currentIP) currentIP.innerHTML = t.currentIP + '<span style="color: #e67e22;">✅ ' + customIPInfo + t.customIPConfig + '</span>';
                            if (regionMatch) regionMatch.innerHTML = t.regionMatch + '<span style="color: #e67e22;">⚠️ ' + t.customIPModeDisabled + '</span>';
                        ...
                        detectedRegion = data.region;
                            ...
                            geoInfo.innerHTML = t.detectionMethod + '<span style="color: #27ae60;">' + t.manualRegion + '</span>';
                            regionStatus.innerHTML = t.workerRegion + '<span style="color: #27ae60;">🎯 ' + t.regionNames[detectedRegion] + t.manualRegionDesc + '</span>';
                        
                        // 显示配置状态而不是检测状态
                            if (backupStatus) backupStatus.innerHTML = t.proxyIPStatus + '<span style="color: #27ae60;">✅ ' + t.proxyIPAvailable + '</span>';
                            if (currentIP) currentIP.innerHTML = t.currentIP + '<span style="color: #27ae60;">✅ ' + t.smartSelection + '</span>';
                            if (regionMatch) regionMatch.innerHTML = t.regionMatch + '<span style="color: #27ae60;">✅ ' + t.sameRegionIP + '</span>';
                        ...
                        geoInfo.innerHTML = t.detectionMethod + '<span style="color: #27ae60;">' + t.cloudflareDetection + '</span>';
                    
                } catch (e) {
                        geoInfo.innerHTML = t.detectionMethod + '<span style="color: #e74c3c;">' + t.detectionFailed + '</span>';
                }
                
                    regionStatus.innerHTML = t.workerRegion + '<span style="color: #27ae60;">✅ ' + t.regionNames[detectedRegion] + '</span>';
                
                // 直接显示配置状态，不再进行检测
                if (backupStatus) {
                        backupStatus.innerHTML = t.proxyIPStatus + '<span style="color: #27ae60;">✅ ' + t.proxyIPAvailable + '</span>';
                }
                
                if (currentIP) {
                        currentIP.innerHTML = t.currentIP + '<span style="color: #27ae60;">✅ ' + t.smartSelection + '</span>';
                }
                
                if (regionMatch) {
                        regionMatch.innerHTML = t.regionMatch + '<span style="color: #27ae60;">✅ ' + t.sameRegionIP + '</span>';
                }
                    ...
                    document.getElementById('regionStatus').innerHTML = t.workerRegion + '<span style="color: #e74c3c;">❌ ' + t.detectionFailed + '</span>';
                    document.getElementById('geoInfo').innerHTML = t.detectionMethod + '<span style="color: #e74c3c;">❌ ' + t.detectionFailed + '</span>';
                    document.getElementById('backupStatus').innerHTML = t.proxyIPStatus + '<span style="color: #e74c3c;">❌ ' + t.detectionFailed + '</span>';
                    document.getElementById('currentIP').innerHTML = t.currentIP + '<span style="color: #e74c3c;">❌ ' + t.detectionFailed + '</span>';
                    document.getElementById('regionMatch').innerHTML = t.regionMatch + '<span style="color: #e74c3c;">❌ ' + t.detectionFailed + '</span>';
            }
        }
            ...
            try {
                const response = await fetch(apiUrl);
                    ...
                    if (response.status === 503) {
                        // KV未配置
                        document.getElementById('kvStatus').innerHTML = '<span style="color: #e67e22;">' + t.kvDisabled + '</span>';
                    ...
                    const data = await response.json();
                        ...
                        if (data && data.kvEnabled === true) {
                            document.getElementById('kvStatus').innerHTML = '<span style="color: #27ae60;">' + t.kvEnabled + '</span>';
                        ...
                        document.getElementById('kvStatus').innerHTML = '<span style="color: #e67e22;">' + t.kvCheckFailed + '</span>';
                        document.getElementById('configCard').style.display = 'block';
        ...
        function updatePathTypeStatus(cp) {
            ...
            if (cp && cp.trim()) {
                // 使用自定义路径 (d)
                pathTypeStatus.innerHTML = '<div style="color: #27ae60;">使用类型: <strong>自定义路径 (d)</strong></div>' +
                    '<div style="margin-top: 5px; color: #2c3e50;">当前路径: <span style="color: #e67e22;">' + cp + '</span></div>' +
                    '<div style="margin-top: 5px; font-size: 0.9rem; color: #7f8c8d;">访问地址: ' + 
                    (currentUrl.split('/')[0] + '//' + currentUrl.split('/')[2]) + cp + '/sub</div>';
            } else {
                // 使用 UUID (u)
                pathTypeStatus.innerHTML = '<div style="color: #27ae60;">使用类型: <strong>UUID 路径 (u)</strong></div>' +
                    '<div style="margin-top: 5px; color: #2c3e50;">当前路径: <span style="color: #e67e22;">' + (currentPath || '(UUID)') + '</span></div>' +
                    '<div style="margin-top: 5px; font-size: 0.9rem; color: #7f8c8d;">访问地址: ' + currentUrl.split('/sub')[0] + '/sub</div>';
            }
        }
        ...
        function updateWkRegionState() {
            ...
            if (customIPInput && wkRegion) {
                ...
                if (hasCustomIP) {
                    ...
                    wkRegion.style.backgroundColor = '#f8f9fa';
                    // 显示提示信息
                    if (wkRegionHint) {
                        wkRegionHint.style.display = 'block';
                        wkRegionHint.style.color = '#e67e22';
                    }
                } else {
        ...
        function showStatus(message, type) {
            ...
            statusDiv.style.borderColor = type === 'success' ? '#27ae60' : '#e74c3c';
